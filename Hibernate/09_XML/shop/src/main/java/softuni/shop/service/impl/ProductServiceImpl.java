...
public class ProductServiceImpl implements ProductService {
    ...
    public void seedProduct(ProductSeedDto[] productSeedDtos) {
        ...
        for (ProductSeedDto productSeedDto : productSeedDtos) {
            ...
            validateAndSave(productSeedDto);
        }
    }

    @Override
    public void importProductsXml(ProductsImporRootDto productsImporRootDto) {
        if (this.productRepository.count() != 0) {
            return;
        }

        for (ProductSeedDto productSeedDto : productsImporRootDto.getProducts()) {

            validateAndSave(productSeedDto);
        }
    }

    private void validateAndSave(ProductSeedDto productSeedDto) {
    ...
    public List<ProductInRangeDto> productsInRange(BigDecimal more, BigDecimal less) {
        ...
        for (Product pe : productEntities) {
            ...
            productInRangeDto.setSeller(
                    String.format("%s %s",
                            pe.getSeller().getFirstName(),
                            pe.getSeller().getLastName())
            );
    ...
    @Override
    public ProductInRangeImportRootDto exportProductsInRangeXml() {
        List<ProductInRangeDto> products = this.productsInRange(BigDecimal.valueOf(500), BigDecimal.valueOf(1000));
        ProductInRangeImportRootDto productInRangeImportRootDto = new ProductInRangeImportRootDto();
        productInRangeImportRootDto.setProducts(products);
        return productInRangeImportRootDto;
    }
