...
public class PartServiceImpl implements PartService {
    ...
    @Override
    public void importPartsXML(PartImportRootDto partImportRootDto) {
        if (this.partRepository.count() != 0) {
            return;
        }

        for ( PartSeedDto partSeedDto : partImportRootDto.getPart()) {
            validAndSavePart(partSeedDto);
        }
    }

    private void validAndSavePart(PartSeedDto partSeedDto) {
        if (!this.validatorUtil.isValid(partSeedDto)) {
            this.validatorUtil.violations(partSeedDto)
                    .forEach(e -> System.out.println(e.getMessage()));
            return;
